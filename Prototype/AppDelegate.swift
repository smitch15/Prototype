...
class AppDelegate: UIResponder, UIApplicationDelegate, WCSessionDelegate, NSURLSessionDelegate, NSURLSessionTaskDelegate, NSURLSessionDataDelegate{
    ...
    let username = "8fda6a80-6679-43f2-8f2e-dcabd7070411"
    let password = "RTsDscPxTJKz"
    ...
    func alchemyWork(){
        ...
        let conf = NSURLSessionConfiguration.defaultSessionConfiguration()
        let session = NSURLSession(configuration: conf, delegate: self, delegateQueue: NSOperationQueue.mainQueue())
        let alchemyURL = "http://gateway-a.watsonplatform.net/calls/text/TextGetCombinedData?apikey=9e375e794e698d99635021aa6f7931fa5bdca62c&outputMode=json&extract=entities,keywords,taxonomy,concepts,relations,doc-sentiment,doc-emotion,typed-rels,authors"
